...
class Controller_View extends Controller {
	...
	/** @var Kostache_Layout */
	protected $_view;
	/** @var string Text name of the corresponding View class. */
	...
	/** @var array Any scripts that get appended to the page. */
	protected $_scripts = [];

	public function before() {
		parent::before();

		$this->_view_class = "View_" . $this->request->controller() . '_' . ucfirst($this->request->action());
		
		if( Kohana::find_file('classes', str_replace('_','/',$this->_view_class)) ){
	...
	/**
	 * Add a script to the main request body (footer).
	 * 
	 * This is meant to allow javascript files to be dynamically called as they are in Core Plus.
	 * 
	 * @param $src
	 */
	public function addScript($src){
		if(strpos($src, '://') === false){
			// Only perform the manipulations if the incoming script is not already fully resolved.
			// Otherwise, the src is passed through verbatim.
			if(strpos($src, ROOT_WDIR) !== 0){
				// The src does NOT begin with the root web directory, so prefix that!
				$src = ROOT_WDIR . ($src{0} == '/' ? substr($src, 1) : $src);
			}
			
			// The source should always have a version string to prevent cache issues.
